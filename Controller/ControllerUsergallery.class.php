...
class ControllerUsergallery extends Controller
{
	...
	private static function displayCom($img_path)
	{
		...
		$req = self::$sel->query_select('*', 'comments', $condition, false, 'date', null, $attributes);
	...
	public function infiniteScroll()
	{
		...
		$id = self::$sel->query_select('id', 'posts', $condition, true, null, null, $attributes);
		...
		if (isset($_SESSION['auth']) && !empty($_SESSION['auth'])) {
			...
			$liked_by_user = self::$sel->query_select('id', 'likes', $conditions, true, null, null, $attributes);
		...
		$count = self::$sel->query_select($value, 'likes', $condition, true, null, null, $attributes);
		$info['countLikes'] = $count['countLikes'];

		//Get Comments
		$comments = self::$sel->query_select('login, img_comment', 'comments', $condition, false, 'id', null, $attributes);
	...
	public static function five_imgs($begin, $form)
	{
		...
		if (isset($_SESSION['auth']) && !empty($_SESSION['auth']))
		{
			...
			$likes = self::$sel->query_select('img_path', 'likes', $condition, false, null, null, $attributes);
		}
		$value = "Count(id) AS 'countLikes'";
		...
		while ($begin < $finish && isset(self::$posts[$begin]))
		{
			...
			$attributes = array(
									self::$posts[$begin]['image_path']
								);
			$req = self::$sel->query_select($value, 'likes', $condition, true, null, null, $attributes);
	...
	public function like(){
		if (isset($_POST['image_path']) && !empty($_POST['image_path'])) {
			...
			self::$ins->insert_value('likes', $values, $attributes);
		}
	}
	...
	public function showLikers()
	{
		if (isset($_POST['image_path']) && !empty($_POST['image_path']))
		{
			...
			$userWhoLiked = self::$sel->query_select('login', 'likes', $condition, false, null, null, $attributes);
			$ret = array ();
	...
	public function comment()
	{
		if (isset($_POST['comment']) && !empty($_POST['comment']))
		{
			...
			$req = self::$sel->query_select('login', 'posts', $condition, true, null, null, $attributes);
			...
			if ($req['login'] !== $_SESSION['auth'])
			{
				...
				$q2 = self::$sel->query_select('email', 'users',  $condition, true, null, null, $attributes);
